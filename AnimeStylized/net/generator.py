...
def Conv2D(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias=None):
    if kernel_size == 3 and strides == 1:
        inputs = tf.pad(
            inputs, [[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
    if kernel_size == 7 and strides == 1:
        inputs = tf.pad(
            inputs, [[0, 0], [3, 3], [3, 3], [0, 0]], mode="REFLECT")
    if strides == 2:
        inputs = tf.pad(
            inputs, [[0, 0], [0, 1], [0, 1], [0, 0]], mode="REFLECT")
...
def Conv2DTransposeLReLU(inputs, filters, kernel_size=2, strides=2, padding='SAME', Use_bias=None):
    ...
    return tf_contrib.layers.conv2d_transpose(inputs,
                                              num_outputs=filters,
                                              kernel_size=kernel_size,
                                              stride=strides,
                                              biases_initializer=Use_bias,
                                              normalizer_fn=tf_contrib.layers.instance_norm,
                                              activation_fn=lrelu,
                                              padding=padding)
...
def Separable_conv2d(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias=tf.zeros_initializer()):
    ...
    if strides == 2:
        inputs = tf.pad(
            inputs, [[0, 0], [0, 1], [0, 1], [0, 0]], mode="REFLECT")
    return tf_contrib.layers.separable_conv2d(
        inputs,
        num_outputs=filters,
        kernel_size=kernel_size,
        depth_multiplier=1,
        ...
        weights_initializer=tf_layers.variance_scaling_initializer(),
        biases_initializer=Use_bias,
        normalizer_fn=tf_contrib.layers.layer_norm,
        activation_fn=lrelu,
        padding=padding)
